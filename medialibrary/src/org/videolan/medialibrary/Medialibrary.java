...
public class Medialibrary {
  ...
  private final List<OnMedialibraryReadyListener> onMedialibraryReadyListeners =
      new ArrayList<>();
  private volatile boolean isMedialibraryStarted = false;
  private final List<DevicesDiscoveryCb> devicesDiscoveryCbList =
      new ArrayList<>();
  private final List<EntryPointsEventsCb> entryPointsEventsCbList =
      new ArrayList<>();
  ...
  public int init(Context context) {
    ...
    if (extFilesDir == null || !extFilesDir.exists() || dbDirectory == null ||
        !dbDirectory.canWrite())
      return ML_INIT_FAILED;
    ...
    int initCode = nativeInit(dbDirectory + VLC_MEDIA_DB_NAME,
                              extFilesDir + THUMBS_FOLDER_NAME);
  ...
  public void start() {
    ...
    nativeSetMediaAddedCbFlag(FLAG_MEDIA_ADDED_AUDIO_EMPTY |
                              FLAG_MEDIA_ADDED_VIDEO_EMPTY);
    nativeSetMediaUpdatedCbFlag(FLAG_MEDIA_UPDATED_AUDIO_EMPTY |
                                FLAG_MEDIA_UPDATED_VIDEO_EMPTY);
  }

  public boolean isStarted() { return isMedialibraryStarted; }
  ...
  public boolean addDevice(@NonNull String uuid, @NonNull String path,
                           boolean removable) {
    return nativeAddDevice(Tools.encodeVLCMrl(uuid), Tools.encodeVLCMrl(path),
                           removable);
  }
  ...
  public boolean removeDevice(String uuid) {
    return mIsInitiated && !TextUtils.isEmpty(uuid) &&
        nativeRemoveDevice(Tools.encodeVLCMrl(uuid));
  }
  ...
  public MediaWrapper[] getVideos(int sort, boolean desc) {
    return mIsInitiated ? nativeGetSortedVideos(sort, desc)
                        : new MediaWrapper[0];
  }
  ...
  public MediaWrapper[] getAudio(int sort, boolean desc) {
    return mIsInitiated ? nativeGetSortedAudio(sort, desc)
                        : new MediaWrapper[0];
  }

  @WorkerThread
  public MediaWrapper[] getPagedAudio(int sort, boolean desc, int nbItems,
                                      int offset) {
    return mIsInitiated ? nativeGetSortedPagedAudio(sort, desc, nbItems, offset)
                        : new MediaWrapper[0];
  }
  ...
  @NonNull
  @WorkerThread
  public Album[] getPagedAlbums(int sort, boolean desc, int nbItems,
                                int offset) {
    return mIsInitiated ? nativeGetPagedAlbums(sort, desc, nbItems, offset)
                        : new Album[0];
  }
  ...
  @WorkerThread
  public Artist[] getPagedArtists(boolean all, int sort, boolean desc,
                                  int nbItems, int offset) {
    return mIsInitiated
        ? nativeGetPagedArtists(all, sort, desc, nbItems, offset)
        : new Artist[0];
  }
  ...
  @NonNull
  @WorkerThread
  public Genre[] getPagedGenres(int sort, boolean desc, int nbItems,
                                int offset) {
    return mIsInitiated ? nativeGetPagedGenres(sort, desc, nbItems, offset)
                        : new Genre[0];
  }
  ...
  @WorkerThread
  public Playlist[] getPagedPlaylists(int sort, boolean desc, int nbItems,
                                      int offset) {
    return mIsInitiated ? nativeGetPagedPlaylists(sort, desc, nbItems, offset)
                        : new Playlist[0];
  }
  ...
  public Playlist createPlaylist(String name) {
    return mIsInitiated && !TextUtils.isEmpty(name) ? nativePlaylistCreate(name)
                                                    : null;
  }
  ...
  public boolean clearHistory() { return mIsInitiated && nativeClearHistory(); }

  public boolean addToHistory(String mrl, String title) {
    return mIsInitiated &&
        nativeAddToHistory(Tools.encodeVLCMrl(mrl), Tools.encodeVLCMrl(title));
  }
  ...
  public MediaWrapper getMedia(Uri uri) {
    ...
    return mIsInitiated && !TextUtils.isEmpty(vlcMrl)
        ? nativeGetMediaFromMrl(vlcMrl)
        : null;
  }
  ...
  public MediaWrapper addMedia(String mrl) {
    final String vlcMrl = Tools.encodeVLCMrl(mrl);
    return mIsInitiated && !TextUtils.isEmpty(vlcMrl) ? nativeAddMedia(vlcMrl)
                                                      : null;
  }
  ...
  public MediaWrapper addStream(String mrl, String title) {
    ...
    return mIsInitiated && !TextUtils.isEmpty(vlcMrl)
        ? nativeAddStream(vlcMrl, vlcTitle)
        : null;
  }

  public void requestThumbnail(long id) {
    if (mIsInitiated)
      nativeRequestThumbnail(id);
  }

  public long getId() { return mInstanceID; }

  public boolean isWorking() { return mIsWorking; }

  public boolean isInitiated() { return mIsInitiated; }
  ...
  public void onPlaylistsAdded() {
    synchronized (mPlaylistCbs) {
      for (PlaylistsCb cb : mPlaylistCbs)
        cb.onPlaylistsAdded();
    }
  }
  ...
  public void onBackgroundTasksIdleChanged(boolean isIdle) {
    ...
    LocalBroadcastManager.getInstance(sContext).sendBroadcast(
        new Intent(ACTION_IDLE).putExtra(STATE_IDLE, isIdle));
    if (isIdle) {
      synchronized (onMedialibraryReadyListeners) {
        for (OnMedialibraryReadyListener listener :
             onMedialibraryReadyListeners)
          listener.onMedialibraryIdle();
      }
    }
  }
  ...
  void onMediaThumbnailReady(MediaWrapper media, boolean success) {
    if (success)
      ((MutableLiveData)lastThumb).postValue(media);
  }
  ...
  public void addArtistsCb(ArtistsCb artistsAddedCb) {
    if (!mIsInitiated)
      return;
    synchronized (mArtistsCbs) { mArtistsCbs.add(artistsAddedCb); }
  }

  public void removeArtistsCb(ArtistsCb artistsAddedCb) {
    if (!mIsInitiated)
      return;
    synchronized (mArtistsCbs) { mArtistsCbs.remove(artistsAddedCb); }
  }

  public void addAlbumsCb(AlbumsCb AlbumsAddedCb) {
    if (!mIsInitiated)
      return;
    synchronized (mAlbumsCbs) { mAlbumsCbs.add(AlbumsAddedCb); }
  }

  public void removeAlbumsCb(AlbumsCb AlbumsAddedCb) {
    if (!mIsInitiated)
      return;
    synchronized (mAlbumsCbs) { mAlbumsCbs.remove(AlbumsAddedCb); }
  }

  public void addGenreCb(GenresCb GenreCb) {
    if (!mIsInitiated)
      return;
    synchronized (mGenreCbs) { this.mGenreCbs.add(GenreCb); }
  }

  public void removeGenreCb(GenresCb GenreCb) {
    if (!mIsInitiated)
      return;
    synchronized (mGenreCbs) { this.mGenreCbs.remove(GenreCb); }
  }

  public void addPlaylistCb(PlaylistsCb playlistCb) {
    if (!mIsInitiated)
      return;
    synchronized (mPlaylistCbs) { this.mPlaylistCbs.add(playlistCb); }
  }

  public void removePlaylistCb(PlaylistsCb playlistCb) {
    if (!mIsInitiated)
      return;
    synchronized (mPlaylistCbs) { this.mPlaylistCbs.remove(playlistCb); }
  }

  public SearchAggregate search(String query) {
    return mIsInitiated && !TextUtils.isEmpty(query) ? nativeSearch(query)
                                                     : null;
  }

  public MediaWrapper[] searchMedia(String query) {
    return mIsInitiated && !TextUtils.isEmpty(query) ? nativeSearchMedia(query)
                                                     : null;
  }

  public MediaWrapper[] searchMedia(String query, int sort, boolean desc,
                                    int nbItems, int offset) {
    return mIsInitiated && !TextUtils.isEmpty(query)
        ? nativeSearchPagedMedia(query, sort, desc, nbItems, offset)
        : null;
  }
  ...
  public MediaWrapper[] searchAudio(String query, int sort, boolean desc,
                                    int nbItems, int offset) {
    return mIsInitiated && !TextUtils.isEmpty(query)
        ? nativeSearchPagedAudio(query, sort, desc, nbItems, offset)
        : null;
  }
  ...
  public MediaWrapper[] searchVideo(String query, int sort, boolean desc,
                                    int nbItems, int offset) {
    return mIsInitiated && !TextUtils.isEmpty(query)
        ? nativeSearchPagedVideo(query, sort, desc, nbItems, offset)
        : null;
  }
  ...
  public Artist[] searchArtist(String query) {
    return mIsInitiated && !TextUtils.isEmpty(query) ? nativeSearchArtist(query)
                                                     : null;
  }

  public Artist[] searchArtist(String query, int sort, boolean desc,
                               int nbItems, int offset) {
    return mIsInitiated && !TextUtils.isEmpty(query)
        ? nativeSearchPagedArtist(query, sort, desc, nbItems, offset)
        : new Artist[0];
  }

  public Album[] searchAlbum(String query, int sort, boolean desc, int nbItems,
                             int offset) {
    return mIsInitiated && !TextUtils.isEmpty(query)
        ? nativeSearchPagedAlbum(query, sort, desc, nbItems, offset)
        : null;
  }

  public Album[] searchAlbum(String query) {
    return mIsInitiated && !TextUtils.isEmpty(query) ? nativeSearchAlbum(query)
                                                     : null;
  }

  public Genre[] searchGenre(String query) {
    return mIsInitiated && !TextUtils.isEmpty(query) ? nativeSearchGenre(query)
                                                     : null;
  }

  public Genre[] searchGenre(String query, int sort, boolean desc, int nbItems,
                             int offset) {
    return mIsInitiated && !TextUtils.isEmpty(query)
        ? nativeSearchPagedGenre(query, sort, desc, nbItems, offset)
        : null;
  }

  public Playlist[] searchPlaylist(String query) {
    return mIsInitiated && !TextUtils.isEmpty(query)
        ? nativeSearchPlaylist(query)
        : null;
  }

  public Playlist[] searchPlaylist(String query, int sort, boolean desc,
                                   int nbItems, int offset) {
    return mIsInitiated && !TextUtils.isEmpty(query)
        ? nativeSearchPagedPlaylist(query, sort, desc, nbItems, offset)
        : null;
  }
  ...
  public void removeDeviceDiscoveryCb(DevicesDiscoveryCb cb) {
    synchronized (devicesDiscoveryCbList) { devicesDiscoveryCbList.remove(cb); }
  }
  ...
  public void
  removeOnMedialibraryReadyListener(OnMedialibraryReadyListener cb) {
    synchronized (onMedialibraryReadyListeners) {
  ...
  private native boolean nativeAddDevice(String uuid, String path,
                                         boolean removable);
  ...
  private native MediaWrapper[] nativeGetSortedPagedAudio(int sort,
                                                          boolean desc,
                                                          int nbItems,
                                                          int offset);
  ...
  private native Album[] nativeGetPagedAlbums(int sort, boolean desc,
                                              int nbItems, int offset);
  ...
  private native Artist[] nativeGetPagedArtists(boolean all, int sort,
                                                boolean desc, int nbItems,
                                                int offset);
  ...
  private native Genre[] nativeGetPagedGenres(int sort, boolean desc,
                                              int nbItems, int offset);
  ...
  private native Playlist[] nativeGetPagedPlaylists(int sort, boolean desc,
                                                    int nbItems, int offset);
