...
static void jni_detach_thread(void *data) { myVm->DetachCurrentThread(); }
...
AndroidMediaLibrary::initML(const std::string &dbPath,
                            const std::string &thumbsPath) {
  p_DeviceListerCb = p_ml->setDeviceLister(p_lister);
  return p_ml->initialize(dbPath, thumbsPath, this);
}

void AndroidMediaLibrary::start() {
  p_ml->start();
  m_started = true;
}

bool AndroidMediaLibrary::addDevice(const std::string &uuid,
                                    const std::string &path, bool removable) {
  p_lister->addDevice(uuid, path, removable);
  return p_DeviceListerCb != nullptr &&
         (m_started ? p_DeviceListerCb->onDevicePlugged(uuid, path)
...
bool AndroidMediaLibrary::removeDevice(const std::string &uuid) {
...
void AndroidMediaLibrary::banFolder(const std::string &path) {
  p_ml->banFolder(path);
}

void AndroidMediaLibrary::unbanFolder(const std::string &path) {
  p_ml->unbanFolder(path);
}

void AndroidMediaLibrary::discover(const std::string &libraryPath) {
  p_ml->discover(libraryPath);
}

void AndroidMediaLibrary::removeEntryPoint(const std::string &entryPoint) {
  p_ml->removeEntryPoint(entryPoint);
}

std::vector<medialibrary::FolderPtr> AndroidMediaLibrary::entryPoints() {
  return p_ml->entryPoints()->all();
}

void AndroidMediaLibrary::pauseBackgroundOperations() {
  p_ml->pauseBackgroundOperations();
  m_paused = true;
}

void AndroidMediaLibrary::setMediaUpdatedCbFlag(int flags) {
  m_mediaUpdatedType = flags;
}

void AndroidMediaLibrary::setMediaAddedCbFlag(int flags) {
  m_mediaAddedType = flags;
}

void AndroidMediaLibrary::resumeBackgroundOperations() {
  p_ml->resumeBackgroundOperations();
  m_paused = false;
}

void AndroidMediaLibrary::reload() { p_ml->reload(); }

void AndroidMediaLibrary::reload(const std::string &entryPoint) {
  p_ml->reload(entryPoint);
}

void AndroidMediaLibrary::forceParserRetry() { p_ml->forceParserRetry(); }

void AndroidMediaLibrary::forceRescan() { p_ml->forceRescan(); }

bool AndroidMediaLibrary::increasePlayCount(int64_t mediaId) {
...
std::vector<medialibrary::MediaPtr> AndroidMediaLibrary::lastMediaPlayed() {
  return p_ml->history()->items(100, 0);
}

bool AndroidMediaLibrary::addToHistory(const std::string &mrl,
...
std::vector<medialibrary::MediaPtr> AndroidMediaLibrary::lastStreamsPlayed() {
  return p_ml->streamHistory()->items(100, 0);
}

bool AndroidMediaLibrary::clearHistory() { return p_ml->clearHistory(); }
...
AndroidMediaLibrary::searchMedia(const std::string &query,
                                 const medialibrary::QueryParameters *params) {
  return p_ml->searchMedia(query, params);
}

medialibrary::Query<medialibrary::IMedia>
AndroidMediaLibrary::searchAudio(const std::string &query,
                                 const medialibrary::QueryParameters *params) {
  return p_ml->searchAudio(query, params);
}

medialibrary::Query<medialibrary::IMedia>
AndroidMediaLibrary::searchVideo(const std::string &query,
                                 const medialibrary::QueryParameters *params) {
  return p_ml->searchVideo(query, params);
}

medialibrary::Query<medialibrary::IMedia> AndroidMediaLibrary::searchFromAlbum(
    int64_t albumId, const std::string &query,
    const medialibrary::QueryParameters *params) {
  auto album = p_ml->album(albumId);
  return album == nullptr ? nullptr : album->searchTracks(query, params);
}

medialibrary::Query<medialibrary::IMedia> AndroidMediaLibrary::searchFromArtist(
    int64_t artistId, const std::string &query,
    const medialibrary::QueryParameters *params) {
...
AndroidMediaLibrary::searchAlbumsFromArtist(
    int64_t artistId, const std::string &query,
    const medialibrary::QueryParameters *params) {
  auto artist = p_ml->artist(artistId);
...
medialibrary::Query<medialibrary::IMedia> AndroidMediaLibrary::searchFromGenre(
    int64_t genreId, const std::string &query,
    const medialibrary::QueryParameters *params) {
...
medialibrary::Query<medialibrary::IAlbum>
AndroidMediaLibrary::searchAlbumsFromGenre(
    int64_t genreId, const std::string &query,
    const medialibrary::QueryParameters *params) {
  auto genre = p_ml->genre(genreId);
...
medialibrary::Query<medialibrary::IMedia>
AndroidMediaLibrary::searchFromPLaylist(
    int64_t playlistId, const std::string &query,
    const medialibrary::QueryParameters *params) {
...
AndroidMediaLibrary::searchPlaylists(
    const std::string &query, const medialibrary::QueryParameters *params) {
...
medialibrary::Query<medialibrary::IArtist> AndroidMediaLibrary::searchArtists(
    const std::string &query, const medialibrary::QueryParameters *params) {
  return p_ml->searchArtists(query, true, params);
}

medialibrary::MediaPtr AndroidMediaLibrary::media(long id) {
  return p_ml->media(id);
}

medialibrary::MediaPtr AndroidMediaLibrary::media(const std::string &mrl) {
  return p_ml->media(mrl);
}

medialibrary::MediaPtr AndroidMediaLibrary::addMedia(const std::string &mrl) {
  return p_ml->addExternalMedia(mrl);
}

medialibrary::MediaPtr
AndroidMediaLibrary::addStream(const std::string &mrl,
                               const std::string &title) {
...
medialibrary::AlbumPtr AndroidMediaLibrary::album(int64_t albumId) {
...
medialibrary::ArtistPtr AndroidMediaLibrary::artist(int64_t artistId) {
...
medialibrary::GenrePtr AndroidMediaLibrary::genre(int64_t genreId) {
...
medialibrary::PlaylistPtr AndroidMediaLibrary::playlist(int64_t playlistId) {
...
medialibrary::Query<medialibrary::IMedia> AndroidMediaLibrary::tracksFromAlbum(
    int64_t albumId, const medialibrary::QueryParameters *params) {
  auto album = p_ml->album(albumId);
  return album == nullptr ? nullptr : album->tracks(params);
}

medialibrary::Query<medialibrary::IMedia> AndroidMediaLibrary::mediaFromArtist(
    int64_t artistId, const medialibrary::QueryParameters *params) {
  auto artist = p_ml->artist(artistId);
  return artist == nullptr ? nullptr : artist->tracks(params);
}

medialibrary::Query<medialibrary::IAlbum> AndroidMediaLibrary::albumsFromArtist(
    int64_t artistId, const medialibrary::QueryParameters *params) {
  auto artist = p_ml->artist(artistId);
  return artist == nullptr ? nullptr : artist->albums(params);
}

medialibrary::Query<medialibrary::IMedia> AndroidMediaLibrary::mediaFromGenre(
    int64_t genreId, const medialibrary::QueryParameters *params) {
  auto genre = p_ml->genre(genreId);
  return genre == nullptr ? nullptr : genre->tracks(params);
}

medialibrary::Query<medialibrary::IAlbum> AndroidMediaLibrary::albumsFromGenre(
    int64_t genreId, const medialibrary::QueryParameters *params) {
  auto genre = p_ml->genre(genreId);
...
medialibrary::Query<medialibrary::IArtist>
AndroidMediaLibrary::artistsFromGenre(
    int64_t genreId, const medialibrary::QueryParameters *params) {
  auto genre = p_ml->genre(genreId);
...
bool AndroidMediaLibrary::playlistAppend(int64_t playlistId, int64_t mediaId) {
  medialibrary::PlaylistPtr playlist = p_ml->playlist(playlistId);
  return playlist == nullptr ? false : playlist->append(mediaId);
}

bool AndroidMediaLibrary::playlistAdd(int64_t playlistId, int64_t mediaId,
                                      unsigned int position) {
  medialibrary::PlaylistPtr playlist = p_ml->playlist(playlistId);
  return playlist == nullptr ? false : playlist->add(mediaId, position);
}

bool AndroidMediaLibrary::playlistMove(int64_t playlistId, int64_t mediaId,
                                       unsigned int position) {
  medialibrary::PlaylistPtr playlist = p_ml->playlist(playlistId);
  return playlist == nullptr ? false : playlist->move(mediaId, position);
}

bool AndroidMediaLibrary::playlistRemove(int64_t playlistId, int64_t mediaId) {
  medialibrary::PlaylistPtr playlist = p_ml->playlist(playlistId);
  return playlist == nullptr ? false : playlist->remove(mediaId);
}

bool AndroidMediaLibrary::PlaylistDelete(int64_t playlistId) {
  return p_ml->deletePlaylist(playlistId);
}

void AndroidMediaLibrary::requestThumbnail(int64_t media_id) {
  medialibrary::MediaPtr media = p_ml->media(media_id);
  if (media != nullptr)
    p_ml->requestThumbnail(media);
}

void AndroidMediaLibrary::onMediaAdded(
    std::vector<medialibrary::MediaPtr> mediaList) {
  if (m_mediaAddedType &
      (FLAG_MEDIA_ADDED_AUDIO | FLAG_MEDIA_ADDED_VIDEO |
       FLAG_MEDIA_ADDED_AUDIO_EMPTY | FLAG_MEDIA_ADDED_VIDEO_EMPTY)) {
    JNIEnv *env = getEnv();
    ...
    int index;
    if ((m_mediaAddedType &
         (FLAG_MEDIA_ADDED_AUDIO | FLAG_MEDIA_ADDED_VIDEO)) == 0) {
      index = 0;
      mediaRefs = (jobjectArray)env->NewObjectArray(
          0, p_fields->MediaWrapper.clazz, NULL);
    } else {
      mediaRefs = (jobjectArray)env->NewObjectArray(
          mediaList.size(), p_fields->MediaWrapper.clazz, NULL);
      index = -1;
      jobject item;
      for (medialibrary::MediaPtr const &media : mediaList) {
        ...
        if ((type == medialibrary::IMedia::Type::Audio &&
             m_mediaAddedType & FLAG_MEDIA_ADDED_AUDIO) ||
            (type == medialibrary::IMedia::Type::Video &&
             m_mediaAddedType & FLAG_MEDIA_ADDED_VIDEO))
    ...
    if (index > -1) {
      if (weak_thiz) {
        results =
            filteredArray(env, mediaRefs, p_fields->MediaWrapper.clazz, -1);
        env->CallVoidMethod(weak_thiz, p_fields->MediaLibrary.onMediaAddedId,
                            results);
...
void AndroidMediaLibrary::onMediaModified(
    std::vector<medialibrary::MediaPtr> mediaList) {
  if (m_mediaUpdatedType & FLAG_MEDIA_UPDATED_AUDIO ||
      m_mediaUpdatedType & FLAG_MEDIA_UPDATED_VIDEO ||
    ...
    jobjectArray mediaRefs, results;
    int index;
    if ((m_mediaUpdatedType &
         (FLAG_MEDIA_UPDATED_AUDIO | FLAG_MEDIA_UPDATED_VIDEO)) == 0) {
      index = 0;
      mediaRefs = (jobjectArray)env->NewObjectArray(
          0, p_fields->MediaWrapper.clazz, NULL);
    } else {
      ...
      for (medialibrary::MediaPtr const &media : mediaList) {
        medialibrary::IMedia::Type type = media->type();
        if ((type == medialibrary::IMedia::Type::Audio &&
             m_mediaUpdatedType & FLAG_MEDIA_UPDATED_AUDIO) ||
            (type == medialibrary::IMedia::Type::Video &&
             m_mediaUpdatedType & FLAG_MEDIA_UPDATED_VIDEO))
          item = mediaToMediaWrapper(env, p_fields, media);
        else
...
void AndroidMediaLibrary::onMediaDeleted(std::vector<int64_t> ids) {
  if (m_mediaAddedType &
      (FLAG_MEDIA_ADDED_AUDIO_EMPTY | FLAG_MEDIA_ADDED_AUDIO |
       FLAG_MEDIA_ADDED_VIDEO | FLAG_MEDIA_ADDED_VIDEO_EMPTY)) {
    JNIEnv *env = getEnv();
    if (env != NULL && weak_thiz)
      env->CallVoidMethod(weak_thiz, p_fields->MediaLibrary.onMediaDeletedId);
  }
}

void AndroidMediaLibrary::onArtistsAdded(
...
void AndroidMediaLibrary::onArtistsModified(
    std::vector<medialibrary::ArtistPtr> artist) {
  if (m_mediaUpdatedType &
      (FLAG_MEDIA_UPDATED_AUDIO | FLAG_MEDIA_UPDATED_AUDIO_EMPTY)) {
    JNIEnv *env = getEnv();
    if (env == NULL)
      return;
    if (weak_thiz) {
      env->CallVoidMethod(weak_thiz,
                          p_fields->MediaLibrary.onArtistsModifiedId);
    }
  }
}
...
void AndroidMediaLibrary::onAlbumsAdded(
    std::vector<medialibrary::AlbumPtr> albums) {
  if (m_mediaAddedType &
      (FLAG_MEDIA_ADDED_AUDIO | FLAG_MEDIA_ADDED_AUDIO_EMPTY)) {
    JNIEnv *env = getEnv();
    if (env == NULL)
      return;
    if (weak_thiz) {
...
void AndroidMediaLibrary::onAlbumsModified(
    std::vector<medialibrary::AlbumPtr> albums) {
  if (m_mediaUpdatedType &
      (FLAG_MEDIA_UPDATED_AUDIO | FLAG_MEDIA_UPDATED_AUDIO_EMPTY)) {
    JNIEnv *env = getEnv();
...
void AndroidMediaLibrary::onPlaylistsAdded(
...
void AndroidMediaLibrary::onPlaylistsModified(
    std::vector<medialibrary::PlaylistPtr> playlist) {
  if (m_mediaUpdatedType &
      (FLAG_MEDIA_UPDATED_AUDIO | FLAG_MEDIA_UPDATED_AUDIO_EMPTY)) {
    JNIEnv *env = getEnv();
    if (env == NULL)
    ...
    if (weak_thiz) {
      env->CallVoidMethod(weak_thiz,
                          p_fields->MediaLibrary.onPlaylistsModifiedId);
    }
  }
}
...
void AndroidMediaLibrary::onPlaylistsDeleted(std::vector<int64_t> ids) {
  if (m_mediaUpdatedType &
    ...
    if (weak_thiz) {
      env->CallVoidMethod(weak_thiz,
                          p_fields->MediaLibrary.onPlaylistsDeletedId);
    }
  }
}
...
void AndroidMediaLibrary::onGenresModified(
...
void AndroidMediaLibrary::onDiscoveryProgress(const std::string &entryPoint) {
  ...
  jstring ep = env->NewStringUTF(entryPoint.c_str());
  if (weak_thiz) {
    env->CallVoidMethod(weak_thiz, p_fields->MediaLibrary.onDiscoveryProgressId,
                        ep);
  }
  env->DeleteLocalRef(ep);
}
...
void AndroidMediaLibrary::onDiscoveryCompleted(const std::string &entryPoint,
  ...
  if (weak_thiz) {
    if (m_progress)
      env->CallVoidMethod(weak_thiz,
                          p_fields->MediaLibrary.onParsingStatsUpdatedId,
                          m_progress);
    env->CallVoidMethod(weak_thiz,
                        p_fields->MediaLibrary.onDiscoveryCompletedId, ep);
  }
  env->DeleteLocalRef(ep);
}
...
void AndroidMediaLibrary::onReloadStarted(const std::string &entryPoint) {
  ...
  jstring ep = env->NewStringUTF(entryPoint.c_str());
  if (weak_thiz) {
    env->CallVoidMethod(weak_thiz, p_fields->MediaLibrary.onReloadStartedId,
                        ep);
  }
  env->DeleteLocalRef(ep);
}
  ...
  if (weak_thiz) {
    if (m_progress)
      env->CallVoidMethod(weak_thiz,
                          p_fields->MediaLibrary.onParsingStatsUpdatedId,
                          m_progress);
    env->CallVoidMethod(weak_thiz, p_fields->MediaLibrary.onReloadCompletedId,
                        ep);
  }
  env->DeleteLocalRef(ep);
}

void AndroidMediaLibrary::onEntryPointBanned(const std::string &entryPoint,
                                             bool success) {
  JNIEnv *env = getEnv();
  if (env == NULL)
    return;
  jstring ep = env->NewStringUTF(entryPoint.c_str());
  if (weak_thiz) {
    env->CallVoidMethod(weak_thiz, p_fields->MediaLibrary.onEntryPointBannedId,
                        ep, success);
  }
  env->DeleteLocalRef(ep);
}

void AndroidMediaLibrary::onEntryPointUnbanned(const std::string &entryPoint,
                                               bool success) {
  JNIEnv *env = getEnv();
  if (env == NULL)
    return;
  ...
  if (weak_thiz) {
    env->CallVoidMethod(
        weak_thiz, p_fields->MediaLibrary.onEntryPointUnbannedId, ep, success);
  }
  env->DeleteLocalRef(ep);
}

void AndroidMediaLibrary::onEntryPointRemoved(const std::string &entryPoint,
                                              bool success) {
  JNIEnv *env = getEnv();
  if (env == NULL)
    return;
  jstring ep = env->NewStringUTF(entryPoint.c_str());
  if (weak_thiz) {
    env->CallVoidMethod(weak_thiz, p_fields->MediaLibrary.onEntryPointRemovedId,
                        ep, success);
  }
  env->DeleteLocalRef(ep);
}
...
void AndroidMediaLibrary::onParsingStatsUpdated(uint32_t percent) {
  ...
  if (weak_thiz) {
    env->CallVoidMethod(
        weak_thiz, p_fields->MediaLibrary.onParsingStatsUpdatedId, progress);
  }
}
...
void AndroidMediaLibrary::onBackgroundTasksIdleChanged(bool isIdle) {
  ...
  if (weak_thiz) {
    env->CallVoidMethod(weak_thiz,
                        p_fields->MediaLibrary.onBackgroundTasksIdleChangedId,
                        isIdle);
  }
}

void AndroidMediaLibrary::onMediaThumbnailReady(medialibrary::MediaPtr media,
                                                bool success) {
  JNIEnv *env = getEnv();
  if (env != NULL && weak_thiz) {
    auto item = mediaToMediaWrapper(env, p_fields, media);
    env->CallVoidMethod(weak_thiz,
                        p_fields->MediaLibrary.onMediaThumbnailReadyId, item,
                        success);
  }
}

JNIEnv *AndroidMediaLibrary::getEnv() {
...
void AndroidMediaLibrary::detachCurrentThread() { myVm->DetachCurrentThread(); }
