...
static struct glrenderer *GLRenderer_getInstance(JNIEnv *env, jobject thiz) {
  struct glrenderer *glr = (struct glrenderer *)(intptr_t)(*env)->GetLongField(
      env, thiz, fields.GLRenderer.mInstanceID);
  if (!glr)
...
static void GLRenderer_setInstance(JNIEnv *env, jobject thiz,
                                   struct glrenderer *glr) {
...
static void glrenderer_destroy_egl(struct glrenderer *glr) {
...
static bool gl_setup(void *opaque) {
...
static void gl_cleanup_locked(struct glrenderer *glr) {
...
static void gl_resize_locked(struct glrenderer *glr, unsigned width,
                             unsigned height) {
...
static void gl_cleanup(void *opaque) {
  struct glrenderer *glr = opaque;
  ...
  if (glr->display) {
    EGLBoolean ret =
        eglMakeCurrent(glr->display, glr->surface, glr->surface, glr->context);
    if (ret) {
      gl_cleanup_locked(glr);
      eglMakeCurrent(glr->display, EGL_NO_SURFACE, EGL_NO_SURFACE,
                     EGL_NO_CONTEXT);
    }
  }
...
static void gl_resize(void *opaque, unsigned width, unsigned height) {
  struct glrenderer *glr = opaque;
...
static void gl_swap(void *opaque) {
  struct glrenderer *glr = opaque;
...
static bool gl_makeCurrent(void *opaque, bool enter) {
  struct glrenderer *glr = opaque;
  ...
  if (!glr->display)
    ...
    ret = eglMakeCurrent(glr->display, EGL_NO_SURFACE, EGL_NO_SURFACE,
                         EGL_NO_CONTEXT);
...
static void *gl_getProcAddress(void *opaque, const char *fct_name) {
  (void)opaque;
  return eglGetProcAddress(fct_name);
}

void Java_org_videolan_libvlc_GLRenderer_nativeInit(JNIEnv *env, jobject thiz,
                                                    jobject thizmp,
                                                    jint egl_version) {
  ...
  libvlc_video_set_opengl_callbacks(mpobj->u.p_mp, libvlc_gl_engine_gles2,
                                    gl_setup, gl_cleanup, gl_resize, gl_swap,
                                    gl_makeCurrent, gl_getProcAddress, glr);

  GLRenderer_setInstance(env, thiz, glr);
}

void Java_org_videolan_libvlc_GLRenderer_nativeRelease(JNIEnv *env,
                                                       jobject thiz) {
...
void Java_org_videolan_libvlc_GLRenderer_nativeOnSurfaceCreated(JNIEnv *env,
                                                                jobject thiz) {
  struct glrenderer *glr = GLRenderer_getInstance(env, thiz);
  if (!glr)
    return;
  ...
  const EGLint config_attr[] = {EGL_RENDERABLE_TYPE,
                                EGL_OPENGL_ES_BIT,
                                EGL_SURFACE_TYPE,
  ...
  const EGLint ctx_attr[] = {EGL_CONTEXT_CLIENT_VERSION, glr->egl_version,
                             EGL_NONE};
  ...
  if (glr->display == EGL_NO_DISPLAY || eglGetError() != EGL_SUCCESS) {
    throw_Exception(env, VLCJNI_EX_RUNTIME, "eglGetCurrentDisplay error: %x",
                    eglGetError());
    goto error;
  }

  if (!eglInitialize(glr->display, NULL, NULL)) {
    throw_Exception(env, VLCJNI_EX_RUNTIME, "eglInitialize() error: %x",
                    eglGetError());
    goto error;
  }
  ...
  if (!eglChooseConfig(glr->display, config_attr, &config, 1, &num_configs) ||
    ...
    throw_Exception(env, VLCJNI_EX_RUNTIME, "eglGetConfigAttrib() error: %x",
                    eglGetError());
    goto error;
  }
  ...
  if (eglGetError() != EGL_SUCCESS) {
    throw_Exception(env, VLCJNI_EX_RUNTIME, "eglGetCurrentContext() error %x",
                    eglGetError());
    goto error;
  }
  ...
  if (glr->context == EGL_NO_CONTEXT || eglGetError() != EGL_SUCCESS) {
    throw_Exception(env, VLCJNI_EX_RUNTIME, "eglCreateContext() error: %x",
                    eglGetError());
    goto error;
  }
...
void Java_org_videolan_libvlc_GLRenderer_nativeOnSurfaceDestroyed(
...
jint Java_org_videolan_libvlc_GLRenderer_nativeGetVideoTexture(JNIEnv *env,
                                                               jobject thiz,
                                                               jobject point) {
  struct glrenderer *glr = GLRenderer_getInstance(env, thiz);
  if (!glr)
  ...
  if (glr->width == 0 || glr->height == 0)
    ...
    if (point != NULL)
      (*env)->CallVoidMethod(env, point, fields.Point.setID, glr->width,
                             glr->height);
  }
