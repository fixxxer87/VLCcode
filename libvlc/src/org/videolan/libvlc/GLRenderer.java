...
public class GLRenderer {
  ...
  GLRenderer(int eglContextClientVersion, MediaPlayer mp,
             MediaPlayer.SurfaceListener listener) {
    if (listener == null)
  ...
  void release() { nativeRelease(); }

  @SuppressWarnings("unused") /* Used from JNI */
  private long mInstance = 0;

  synchronized boolean isValid() { return mSurfaceCreated; }

  /**
   * This method need to be called when a new EGL Context is created (from the
   * same thread).
   *
   * Example: call this method from
   * {@link android.opengl.GLSurfaceView.Renderer#onSurfaceCreated(GL10,
   * EGLConfig)}.
   */
  ...
  /**
   * This method need to be called when the EGL Context is destroyed (from any
   * thread).
   *
   * Example: call this method from {@link
   * android.opengl.GLSurfaceView.Renderer#onPause()}.
   */
   ...
   */
  public boolean isVideoFrameUpdated() { return mFrameUpdated; }
