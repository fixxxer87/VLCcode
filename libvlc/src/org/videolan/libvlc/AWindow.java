...
public class AWindow implements IVLCVout {
  ...
  private class SurfaceHelper {
    ...
    private SurfaceHelper(int id, Surface surface,
    ...
    SurfaceHolder getSurfaceHolder() { return mSurfaceHolder; }
  ...
  private SurfaceTextureThread mSurfaceTextureThread =
      new SurfaceTextureThread();

  /**
   * Create an AWindow
   *
   * You call this directly only if you use the libvlc_media_player native API
   * (and not the Java MediaPlayer class).
  ...
  private void ensureInitState() throws IllegalStateException {
    ...
    if (mSurfacesState.get() != SURFACE_STATE_INIT)
      throw new IllegalStateException(
          ...
          + "mSurfaces[ID_VIDEO]: " + mSurfaceHelpers[ID_VIDEO] + " / " +
          mSurfaces[ID_VIDEO] + ", "
  ...
  private void setSurface(int id, Surface surface,
                          SurfaceHolder surfaceHolder) {
    ensureInitState();
    if (!surface.isValid() && surfaceHolder == null)
      throw new IllegalStateException(
          "surface is not attached and holder is null");
    final SurfaceHelper surfaceHelper = mSurfaceHelpers[id];
    if (surfaceHelper != null)
      surfaceHelper.release();
  ...
  @Override
  public void setSubtitlesSurface(Surface subtitlesSurface,
                                  SurfaceHolder surfaceHolder) {
  ...
  public void attachViews(OnNewVideoLayoutListener onNewVideoLayoutListener) {
    if (mSurfacesState.get() != SURFACE_STATE_INIT ||
        mSurfaceHelpers[ID_VIDEO] == null)
      throw new IllegalStateException(
          "already attached or video view not configured");
  ...
  private void onSurfaceCreated() {
    ...
    if (videoHelper.isReady() &&
        (subtitlesHelper == null || subtitlesHelper.isReady())) {
  ...
  public void sendMouseEvent(int action, int button, int x, int y) {
    synchronized (mNativeLock) {
      if (mCallbackNativeHandle != 0 &&
          (mMouseAction != action || mMouseButton != button || mMouseX != x ||
           mMouseY != y))
  ...
  private void setNativeSurface(int id, Surface surface) {
    synchronized (mNativeLock) { mSurfaces[id] = surface; }
  }

  private Surface getNativeSurface(int id) {
    synchronized (mNativeLock) { return mSurfaces[id]; }
  }
  ...
  private static native void nativeOnMouseEvent(long nativeHandle, int action,
   ...
   */
  @SuppressWarnings("JniMissingFunction")
  private static native void nativeOnWindowSize(long nativeHandle, int width,
                                                int height);
  ...
  private final static int AWINDOW_REGISTER_FLAGS_HAS_VIDEO_LAYOUT_LISTENER =
      0x2;

  /**
   * Set a callback in order to receive {@link #nativeOnMouseEvent} and {@link
   * #nativeOnWindowSize} events.
   *
  ...
  private int registerNative(long nativeHandle) {
    ...
    synchronized (mNativeLock) {
      ...
      if (mMouseAction != -1)
        nativeOnMouseEvent(mCallbackNativeHandle, mMouseAction, mMouseButton,
                           mMouseX, mMouseY);
    ...
    synchronized (mNativeLock) {
      if (mCallbackNativeHandle == 0)
        throw new IllegalArgumentException(
            "unregister called when not registered");
      mCallbackNativeHandle = 0;
    }
  }

  /**
   * This method is only used for HoneyComb and before since
   * ANativeWindow_setBuffersGeometry doesn't work before. It is synchronous.
   *
   ...
   */
  @SuppressWarnings("unused") /* used by JNI */
  private boolean setBuffersGeometry(final Surface surface, final int width,
   ...
   * This call will result of{@link
   * IVLCVout.OnNewVideoLayoutListener#onNewVideoLayout(IVLCVout, int, int, int,
   * int, int, int)} being called from the main thread.
   *
   ...
   */
  @SuppressWarnings("unused") /* used by JNI */
  private void setVideoLayout(final int width, final int height,
                              final int visibleWidth, final int visibleHeight,
                              final int sarNum, final int sarDen) {
    mHandler.post(new Runnable() {
      @Override
      public void run() {
        /* No need to synchronize here, mOnNewVideoLayoutListener is only set
         * from MainThread */
        if (mOnNewVideoLayoutListener != null)
          mOnNewVideoLayoutListener.onNewVideoLayout(
              AWindow.this, width, height, visibleWidth, visibleHeight, sarNum,
              sarDen);
      }
    });
  }
  ...
  private static class SurfaceTextureThread
    ...
    private synchronized boolean attachToGLContext(int texName) {
      /* Try to re-use the same SurfaceTexture until views are detached. By
       * reusing the same SurfaceTexture, we don't have to reconfigure
       * MediaCodec when it signals a video size
    ...
    public void run() {
      ...
      synchronized (this) {
        /* Ideally, all devices are running Android O, and we can create a
         * SurfaceTexture without an OpenGL context and we can specify the
         * thread (Handler) where to run SurfaceTexture callbacks. But this is
         * not the case. The SurfaceTexture has to be created from a new thread
         * with a prepared looper in order to don't use the MainLooper one (and
         * have deadlock when we stop VLC from the mainloop).
         */
    ...
    private synchronized Surface getSurface() { return mSurface; }
   ...
   * calling thread.
   *
   * @param texName the OpenGL texture object name (e.g. generated via
   *     glGenTextures)
  ...
  /**
   * Detach the SurfaceTexture from the OpenGL ES context that owns the OpenGL
   * ES texture object.
   */
  @SuppressWarnings("unused") /* used by JNI */
