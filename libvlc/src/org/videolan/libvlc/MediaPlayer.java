...
public class MediaPlayer extends VLCObject<MediaPlayer.Event> {
  ...
  public static class Event extends VLCEvent {
    ...
    protected Event(int type) { super(type); }
    protected Event(int type, long arg1) { super(type, arg1); }

    protected Event(int type, long arg1, long arg2) { super(type, arg1, arg2); }

    protected Event(int type, float argf) { super(type, argf); }

    public long getTimeChanged() { return arg1; }

    public long getLengthChanged() { return arg1; }

    public float getPositionChanged() { return argf1; }
    public int getVoutCount() { return (int)arg1; }
    public int getEsChangedType() { return (int)arg1; }
    public int getEsChangedID() { return (int)arg2; }
    public boolean getPausable() { return arg1 != 0; }
    public boolean getSeekable() { return arg1 != 0; }
    public float getBuffering() { return argf1; }
  }
  ...
  public static class Title {
    ...
    public boolean isMenu() { return (this.flags & Flags.MENU) != 0; }
  ...
  private static Title createTitleFromNative(long duration, String name,
                                             int flags) {
  ...
  @SuppressWarnings("unused") /* Used from JNI */
  private static Chapter createChapterFromNative(long timeOffset, long duration,
                                                 String name) {
  ...
  @SuppressWarnings("unused") /* Used from JNI */
  private static TrackDescription
  createTrackDescriptionFromNative(int id, String name) {
  ...
  public static class Equalizer {
     ...
     */
    public static Equalizer create() { return new Equalizer(); }

    /**
     * Create a new equalizer, with initial frequency values copied from an
     * existing preset. The new equalizer can subsequently be applied to a media
     * player by invoking
     * {@link MediaPlayer#setEqualizer}.
     */
     ...
     */
    public static int getPresetCount() { return nativeGetPresetCount(); }

    /**
     * Get the name of a particular equalizer preset.
     * This name can be used, for example, to prepare a preset label or menu in
     * a user interface.
     *
     ...
     */
    public static int getBandCount() { return nativeGetBandCount(); }

    /**
     * Get a particular equalizer band frequency.
     * This value can be used, for example, to create a label for an equalizer
     * band control in a user interface.
     *
     ...
     */
    public float getPreAmp() { return nativeGetPreAmp(); }

    /**
     * Set a new pre-amplification value for an equalizer.
     * The new equalizer settings are subsequently applied to a media player by
     * invoking
     * {@link MediaPlayer#setEqualizer}.
     * The supplied amplification value will be clamped to the -20.0 to +20.0
     * range.
     *
     * @param preamp value (-20.0 to 20.0 Hz)
     * @return true on success.
     */
    public boolean setPreAmp(float preamp) { return nativeSetPreAmp(preamp); }
     ...
     */
    public float getAmp(int index) { return nativeGetAmp(index); }
  ...
  private synchronized void updateAudioOutputDevice(long encodingFlags,
                                                    String defaultDevice) {
    mCanDoPassthrough = encodingFlags != 0;
    final String newDeviceId = mAudioDigitalOutputEnabled && mCanDoPassthrough
                                   ? "encoded:" + encodingFlags
                                   : defaultDevice;
    ...
    return new BroadcastReceiver() {
      ...
      public void onReceive(Context context, Intent intent) {
        ...
        if (action.equalsIgnoreCase(AudioManager.ACTION_HDMI_AUDIO_PLUG)) {
          final boolean hasHdmi =
              intent.getIntExtra(AudioManager.EXTRA_AUDIO_PLUG_STATE, 0) == 1;
          final long encodingFlags =
              !hasHdmi ? 0
                       : getEncodingFlags(intent.getIntArrayExtra(
  ...
  private final BroadcastReceiver mAudioPlugReceiver =
      AndroidUtil.isLolliPopOrLater && !AndroidUtil.isMarshMallowOrLater
          ? createAudioPlugReceiver()
          : null;
  ...
  private void registerAudioPlugV21(boolean register) {
    if (register) {
      ...
      final Intent stickyIntent = mLibVLC.mAppContext.registerReceiver(
          mAudioPlugReceiver, intentFilter);
  ...
  private void registerAudioPlugV23(boolean register) {
    AudioManager am = (AudioManager)mLibVLC.mAppContext.getSystemService(
  ...
  public int setRenderer(RendererItem item) { return nativeSetRenderer(item); }

  public synchronized boolean hasMedia() { return mMedia != null; }

  /**
   * Get the Media used by this MediaPlayer. This Media should be released with
   * {@link #release()}.
   */
  ...
  public void play() {
    synchronized (this) {
      if (!mPlaying) {
        /* HACK: stop() reset the audio output, so set it again before first
         * play. */
        if (mAudioReset) {
   ...
   */
  public float getScale() { return nativeGetScale(); }

  /**
   * Set the video scaling factor
   *
   * That is the ratio of the number of pixels on screen to the number of pixels
   * in the original decoded video in each dimension. Zero is a special value;
   * it will adjust the video to the output window/drawable (in windowed mode)
   * or the entire screen.
   *
   * @param scale the scaling factor, or zero
   */
  public void setScale(float scale) { nativeSetScale(scale); }
   ...
   */
  public String getAspectRatio() { return nativeGetAspectRatio(); }
   ...
   */
  public void setAspectRatio(String aspect) { nativeSetAspectRatio(aspect); }
   ...
   * @return true on success.
   */
  public boolean updateViewpoint(float yaw, float pitch, float roll, float fov,
                                 boolean absolute) {
   ...
   *
   * By default, the "android_audiotrack" is selected. Starting Android 21,
   * passthrough is enabled for encodings supported by the device/audio system.
   *
  ...
  public synchronized boolean setAudioOutput(String aout) {
    mAudioOutput = aout;
    /* If The user forced an output different than AudioTrack, don't listen to
     * audio plug events and let the user decide */
   ...
   * If {@link #setAudioOutputDevice} was previously called, this method won't
   * have any effects.
   *
   ...
   *
   * @param encodings list of encodings to play via passthrough (see
  ...
  private synchronized boolean setAudioOutputDeviceInternal(String id,
                                                            boolean fromUser) {
    mAudioOutputDevice = id;
    if (fromUser) {
      /* The user forced a device, don't listen to audio plug events and let the
       * user decide */
   ...
   * Audio output will be moved to the device specified by the device identifier
   * string.
   *
   ...
   * ENCODING_FLAGS_MASK. This extra value is a long that contains
   * binary-shifted AudioFormat.ENCODING_* values.
   *
   * Calling this method will disable the encoding detection (see {@link
   * #setAudioOutput} and {@link #setAudioDigitalOutputEnabled(boolean)}).
   *
   * @return true on success.
   */
   ...
   */
  public Title[] getTitles() { return nativeGetTitles(); }
   ...
   */
  public Chapter[] getChapters(int title) { return nativeGetChapters(title); }

  /**
   * Get the number of available video tracks.
   */
  public int getVideoTracksCount() { return nativeGetVideoTracksCount(); }

  /**
   * Get the list of available video tracks.
   */
  public TrackDescription[] getVideoTracks() { return nativeGetVideoTracks(); }
   ...
   */
  public int getVideoTrack() { return nativeGetVideoTrack(); }
  ...
  public boolean setVideoTrack(int index) {
    ...
    if (index == -1 ||
        (mWindow.areViewsAttached() && !mWindow.areSurfacesWaiting()) ||
        (mGLRenderer != null && mGLRenderer.isValid())) {
   ...
   */
  public int getAudioTracksCount() { return nativeGetAudioTracksCount(); }

  /**
   * Get the list of available audio tracks.
   */
  public TrackDescription[] getAudioTracks() { return nativeGetAudioTracks(); }
   ...
   */
  public int getAudioTrack() { return nativeGetAudioTrack(); }
   ...
   * @return true on success.
   */
  public boolean setAudioTrack(int index) { return nativeSetAudioTrack(index); }
   ...
   */
  public long getAudioDelay() { return nativeGetAudioDelay(); }

  /**
   * Set current audio delay. The audio delay will be reset to zero each time
   * the media changes.
   *
   ...
   */
  public int getSpuTracksCount() { return nativeGetSpuTracksCount(); }

  /**
   * Get the list of available spu (subtitle) tracks.
   */
  public TrackDescription[] getSpuTracks() { return nativeGetSpuTracks(); }
   ...
   */
  public int getSpuTrack() { return nativeGetSpuTrack(); }
   ...
   * @return true on success.
   */
  public boolean setSpuTrack(int index) { return nativeSetSpuTrack(index); }
   ...
   * @return delay in microseconds.
   */
  public long getSpuDelay() { return nativeGetSpuDelay(); }

  /**
   * Set current spu (subtitle) delay. The spu delay will be reset to zero each
   * time the media changes.
   *
   * @param delay in microseconds.
   * @return true on success.
   */
  public boolean setSpuDelay(long delay) { return nativeSetSpuDelay(delay); }
   ...
   *
   * It is possible to apply new equalizer settings to a media player whether
   * the media player is currently playing media or not.
   *
   * Invoking this method will immediately apply the new equalizer settings to
   * the audio output of the currently playing media if there is any.
   *
   * If there is no currently playing media, the new equalizer settings will be
   * applied later if and when new media is played.
   *
   * Equalizer settings will automatically be applied to subsequently played
   * media.
   *
  ...
  @Override
  protected synchronized Event onEventNative(int eventType, long arg1,
                                             long arg2, float argf1) {
    switch (eventType) {
  ...
  public boolean canDoPassthrough() { return mCanDoPassthrough; }
  ...
  private native boolean nativeUpdateViewpoint(float yaw, float pitch,
